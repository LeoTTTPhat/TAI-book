...
protocol (Section~\ref{sec:trust-dashboards}).
...
% ------------------------------------------------------------------
% ------------------------------------------------------------------
\section{The CFO Conversation: Trust ROI Calculator \& Summary}
\label{sec:cfo-summary}
% ------------------------------------------------------------------

The RAV framework culminates in the CFO conversation. To make this dialogue immediately actionable, architects must translate abstract risk into concrete financial sums using jurisdictional benchmarks, and present the final calculation in a standardised worksheet.

\subsection{Cost-per-Record Benchmarks (ASEAN vs. EU)}
\label{subsec:cost-per-record}

To calculate the Loss Exposure $L_m$ for PII-related incident classes (such as the McHire breach), architects must convert the volume of exposed records into a financial sum.

Table~\ref{tab:cost-per-record} provides baseline Cost-per-Record estimates aggregating regulatory fines, legal settlements, and incident response overheads. The drastic discrepancy between the EU and ASEAN highlights why the Trustworthy AI Stack is no longer optional for cross-border deployments.

\begin{table}[ht]
\centering
\caption{Default Cost-per-Record estimates (2025/2026 projected) for PII breaches, segmented by regulatory jurisdiction and data sensitivity. Derived from industry breach reports and adjusted for GDPR and Vietnam Decree 13 penalty structures.}
\label{tab:cost-per-record}
\small
\begin{tabular}{p{3.8cm}rr}
\hline\noalign{\smallskip}
\textbf{Data Sensitivity Tier}
  & \textbf{ASEAN (Decree 13 / PDPA)}
  & \textbf{EU (GDPR / AI Act)} \\
\noalign{\smallskip}\svhline\noalign{\smallskip}
Standard PII (Names, Contact) & \$15 -- \$25 & \$140 -- \$180 \\
Financial (Credit, Txns) & \$45 -- \$70 & \$210 -- \$260 \\
Health / Biometric & \$80 -- \$120 & \$350 -- \$450 \\
\noalign{\smallskip}\hline\noalign{\smallskip}
\end{tabular}
\end{table}

\paragraph{Applying the Benchmarks.}
If a Vietnamese fintech agent exposes 500,000 financial records, the baseline loss $L = 500,000 \times \$55$ (midpoint) = \$27.5M. If that same database includes EU citizens, the GDPR blending pushes the expected loss over \$100M, radically changing the RAV calculation and justifying immediate investment in Differential Privacy and TEEs.

\subsection{The One-Page RAV Summary Worksheet}
\label{subsec:rav-worksheet}

The following template translates the RAV framework output into the exact language a financial decision-maker requires. Architects should populate this template before any budget discussion.

\begin{tcolorbox}[formulabox,
  title={Trust ROI Calculator: CFO Summary Worksheet}]

\textbf{1. Context \& Exposure} \\
\textbf{Organisation:} \underline{\hspace{3.5cm}} \quad \textbf{System:} \underline{\hspace{3.5cm}} \\
\textbf{Data Volume:} \underline{\hspace{2cm}} records \quad \textbf{Jurisdiction(s):} \underline{\hspace{3cm}} \\
\textbf{Target Incident Class:} \underline{\hspace{6.8cm}} \\
\textbf{Calculated Loss Exposure ($L$):} \$ \underline{\hspace{3.5cm}}

\medskip
\hrule
\medskip

\textbf{2. The Trustworthy AI Stack Investment}
\begin{tabular}{@{}p{5cm} r@{}}
Proposed Mechanism(s) & \underline{\hspace{5.5cm}} \\
Total Annual Cost ($\sum C_{m_i}$) & \$ \underline{\hspace{5.5cm}} \\
Current Probability ($p$) & \underline{\hspace{5cm}} \% \\
Residual Risk after Stack ($p_{\mathrm{residual}}^{(k)}$) & \underline{\hspace{5cm}} \% \\
\end{tabular}
...
\textbf{3. The Return on Investment (ROI)}
\begin{tabular}{@{}p{5cm} r@{}}
Expected Loss Reduction & \$ \underline{\hspace{5.5cm}} \\
\textbf{Portfolio RAV $\mathrm{RAV}(M)$} & \textbf{\$ \underline{\hspace{5.5cm}}} \\
\textbf{RAV Ratio ($\rho$)} & \textbf{\underline{\hspace{5.2cm}}$\times$} \\
\end{tabular}

\medskip
\textbf{The Ask:} \\
Approve \$\underline{\hspace{1.5cm}} in annual trust mechanism investment to reduce expected annual loss from \$\underline{\hspace{1.5cm}} to \$\underline{\hspace{1.5cm}}, delivering a \underline{\hspace{0.8cm}}$\times$ return on every security dollar invested.

\medskip
\textbf{The Regulatory Floor:} \\
Under \underline{\hspace{2.5cm}} [Decree~13 / AI Law / EU AI Act], non-compliance carries a mandatory minimum penalty of \$\underline{\hspace{1.5cm}}, making this baseline investment a legal necessity independent of the RAV calculation.
\end{tcolorbox}
...
% ------------------------------------------------------------------
\section{The Sovereign AI Scorecard as Underwriting Evidence}
\label{sec:ai-insurance}
% ------------------------------------------------------------------

A nascent but rapidly maturing market is emerging at the intersection of this book's technical frameworks and the global financial system: \textbf{AI Insurance}. Specialist underwriters at Lloyd's of London, Munich Re, and Beazley began offering dedicated AI liability and cyber-resilience policies in 2025, but face a fundamental actuarial challenge: \emph{how do you price a risk you cannot directly observe?}

The answer lies in the technical artefacts already produced by the Trustworthy AI Stack. The \textbf{Sovereign AI Scorecard} (Table~1.2) and the \textbf{Trust Debt Score} (Section~\ref{sec:trust-debt}) are not merely internal governance tools---they are precisely the kind of structured, verifiable, mechanism-level evidence that actuarial underwriting requires. A system's DP-SGD privacy budget is as observable as a sprinkler system; its ATTM gap score is as auditable as a fire safety inspection report.

\subsection{The Underwriting Evidence Map}
\label{sec:underwriting_map}

Table~\ref{tab:underwriting_evidence} maps each dimension of the Sovereign AI Scorecard to its corresponding insurance underwriting category and the expected premium impact of a strong technical score, based on emerging actuarial frameworks from the 2025--2026 AI insurance market.

\begin{table}[ht]
\centering
\caption{Sovereign AI Scorecard as Actuarial Underwriting Evidence: mapping technical scores to insurance underwriting categories and indicative premium impact.}
\label{tab:underwriting_evidence}
\renewcommand{\arraystretch}{1.4}
\begin{tabular}{p{2.9cm} p{3.6cm} p{3.6cm} p{2.2cm}}
\toprule
\textbf{Scorecard Dimension} & \textbf{Technical Evidence} & \textbf{Underwriting Category} & \textbf{Premium Impact} \\
\midrule
Privacy Guarantee &
DP-SGD with $\varepsilon \leq 2.0$ (Ch.~5);\newline MIA resistance score &
PII breach liability;\newline regulatory fine exposure &
$-15\%$ to $-30\%$ \\

Security Posture &
TEE-protected inference (Ch.~9);\newline SBOM provenance (Ch.~24) &
Cyber incident;\newline supply-chain breach &
$-10\%$ to $-25\%$ \\

Verifiability Score &
ZK-ML fairness proof (Ch.~21);\newline ATTM gap score (Ch.~25) &
Discrimination liability;\newline audit failure &
$-20\%$ to $-35\%$ \\

Alignment Rating &
Constitutional AI + Emergency Brake\newline (Ch.~3, 23) &
Autonomous agent liability;\newline bodily harm coverage &
$-25\%$ to $-40\%$ \\

Trust Debt Score &
ATTM gap analysis (Ch.~25) &
Overall policy deductible tier &
$\pm 10\%$ per tier \\
\bottomrule
\end{tabular}
\end{table}

\subsection{The Trust Debt Score as a Technical Credit Rating}
\label{sec:trust-debt-rating}

The \textbf{Trust Debt Score} functions, in the insurance context, as a \textbf{technical credit rating}---analogous to a FICO score for financial risk, but measuring the accumulated architectural liability of an AI deployment. A system with a high Trust Debt Score (many ATTM gaps unfilled, no formal verification, no DP guarantees) is actuarially equivalent to a building with no fire suppression: the exposure is observable, quantifiable, and directly priceable.

The practical implication for architects is significant. A documented ATTM gap report (Chapter~\ref{chap:attm}) submitted at policy inception can:

\begin{enumerate}
  \item \textbf{Reduce the deductible tier} by demonstrating proactive, structured risk awareness to the underwriter.
  \item \textbf{Unlock incident response coverage} for specific threat classes---Membership Inference Attacks, prompt injection, model inversion---whose mitigations are evidenced in the scorecard.
  \item \textbf{Satisfy the pre-loss audit requirement}, replacing a bespoke third-party technical assessment with the standardized ATTM framework, reducing underwriting friction and cost.
\end{enumerate}

\begin{svgraybox}
\textbf{Regulatory Precedent (2025--2026):} The UK Financial Conduct Authority (FCA) and the Monetary Authority of Singapore (MAS) jointly issued guidance requiring that AI systems used in regulated financial services maintain an auditable ``AI Risk Dossier'' as a condition of professional indemnity coverage. The RAV framework of this appendix, combined with the ATTM scorecard and the Sovereign AI Scorecard, constitutes a complete and compliant AI Risk Dossier under both jurisdictions.
\end{svgraybox}
