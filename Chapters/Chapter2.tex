...
% ------------------------------------------------------------------
\subsection{Worked Trace: Vietnamese Patient Lab Result}
\label{subsec:ci_worked_trace}
% ------------------------------------------------------------------

Abstract parameters become concrete when traced through a realistic data flow. Consider a Vietnamese patient whose HbA1c lab result is generated at a district-level hospital and subsequently requested by a health insurance API. Table~\ref{tab:ci_worked_trace} applies the Five-Parameter model to two flows---the appropriate clinical flow and the CI-violating insurance flow---making visible exactly which parameter shift breaks contextual integrity.

\begin{table}[ht]
\centering
\caption{CI Five-Parameter Trace: Vietnamese patient HbA1c lab result. The clinical flow (rows 1--5) satisfies contextual integrity. The insurance API flow (rows 6--10) violates it at the Recipient and Transmission Principle parameters.}
\label{tab:ci_worked_trace}
\renewcommand{\arraystretch}{1.3}
\begin{tabular}{p{2.4cm} p{3.4cm} p{3.4cm} p{3.1cm}}
\hline\noalign{\smallskip}
\textbf{Parameter} & \textbf{Clinical Flow (Appropriate)} & \textbf{Insurance API Flow (Violation)} & \textbf{CI Status} \\
\noalign{\smallskip}\svhline\noalign{\smallskip}
Subject & Patient (pseudonymized under Decree~13 Art.~9) & Same patient & -- \\[3pt]
Sender & District hospital laboratory system & Hospital AI agent acting autonomously & -- \\[3pt]
Recipient & Attending physician (licensed, duty-of-care) & \cellcolor{costhigh}InsuranceAPI (commercial, no duty-of-care) & \cellcolor{costhigh}VIOLATION \\[3pt]
Information Type & HbA1c value (biometric health data, Class A) & Same value, unmasked & -- \\[3pt]
Transmission Principle & \cellcolor{costlow}Confidentiality + clinical necessity & \cellcolor{costhigh}No consent; purpose not disclosed to patient & \cellcolor{costhigh}VIOLATION \\
\noalign{\smallskip}\hline\noalign{\smallskip}
\end{tabular}
\end{table}

The Gatekeeper Pattern (Section~\ref{sec:ci_agents_gatekeeper}) operationalizes this trace: the PrivacyChecker Middleware evaluates the \emph{Recipient} and \emph{Transmission Principle} parameters at the API boundary and blocks the insurance flow before it exits the hospital's data boundary, triggering the Emergency Brake (Chapter~\ref{chap:hitl}) and logging a CI violation under Decree~13 Article~8.
...
% ------------------------------------------------------------------
\section{CI Violation Taxonomy}
\label{sec:ci_violation_taxonomy}
% ------------------------------------------------------------------

Not all contextual integrity violations are structurally identical. Before designing a Gatekeeper, an architect must classify the type of violation a system is susceptible to, because each type calls for a different technical countermeasure. Table~\ref{tab:ci_violation_taxonomy} defines the four canonical violation classes and maps each to its architectural remedy.

\begin{table}[ht]
\centering
\caption{CI Violation Taxonomy: four violation classes, canonical examples in AI systems, and the primary architectural countermeasure for each.}
\label{tab:ci_violation_taxonomy}
\renewcommand{\arraystretch}{1.3}
\begin{tabular}{p{2.8cm} p{3.2cm} p{2.8cm} p{3.5cm}}
\hline\noalign{\smallskip}
\textbf{Violation Class} & \textbf{Definition} & \textbf{AI Example} & \textbf{Architectural Remedy} \\
\noalign{\smallskip}\svhline\noalign{\smallskip}
Context Collapse & Data shared in one context is exposed to an audience from a different, incompatible context & Employee Slack messages fed to HR sentiment analysis model & Microservice context boundary; PrivacyChecker blocks cross-context recipient \\[4pt]
Purpose Creep & Data collected for purpose $A$ is subsequently used for purpose $B$ without consent & Health app GPS collected for ``fitness'' later sold to insurer & Context-Aware Tagging in data pipeline; purpose token validated at every transformation step \\[4pt]
Proxy Inference & Sensitive attribute $S$ is inferred from a non-sensitive proxy $P$ because $I(P;S) > 0$ & Credit model infers ethnicity from ZIP code & Mutual-information audit at training time; adversarial fairness constraint on $I(P;S)$ \\[4pt]
Cross-Context Aggregation & Individually innocuous data points from $k$ contexts aggregate to reveal a sensitive profile & GPS pings + purchase history + social graph $\Rightarrow$ religious affiliation & $k$-anonymity / DP aggregation at join boundary; prohibition on multi-context JOIN without explicit consent \\
\noalign{\smallskip}\hline\noalign{\smallskip}
\end{tabular}
\end{table}

\begin{svgraybox}
\textbf{Architect's Rule of Thumb:} Context Collapse and Purpose Creep are \emph{flow} violations---they are caught by intercepting a data movement event. Proxy Inference and Cross-Context Aggregation are \emph{structural} violations that cannot be detected by monitoring a single API call; they require model-level auditing (Chapter~\ref{chap:privacy_attacks}) and pipeline-level aggregation controls (Chapter~\ref{chap:k_anonymity}).
\end{svgraybox}
...
\begin{lstlisting}[language=Python, style=pythonstyle,
  caption={PrivacyChecker Middleware: a simplified representation of a
           Contextual Integrity (CI) enforcement loop for autonomous
           AI agents.},
  label={lst:privacy-checker}]
